...
def select_device(device='', apex=False, batch_size=None):
    ...
    if cuda:
        ...
        if ng > 1 and batch_size:  # check that batch_size is compatible with device_count
            assert (
                batch_size % ng == 0
            ), f'batch-size {batch_size:g} not multiple of GPU count {ng:g}'
...
def load_classifier(name='resnet101', n=2):
    ...
    model = getattr(pretrainedmodels, name)(num_classes=1000, pretrained='imagenet')
...
def scale_img(img, r=1.0):  # img(16,3,256,416), r=ratio
    ...
    return (
        F.pad(img, [0, p[1], 0, p[0]], value=0.5) if r < 1.0 else img[:, :, : p[0], : p[1]]
    )  # pad/crop
