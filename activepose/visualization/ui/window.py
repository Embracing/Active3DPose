...
from activepose.env_config import get_map_config
...
class Window(QMainWindow):
    def __init__(self, meta_info, parent=None):
    ...
    def add_view(self):
        ...
        getattr(self.m_ui, groupBox_name).setStyleSheet(
            f'QGroupBox:title {{color: rgb({cam_color[0]}, {cam_color[1]}, {cam_color[2]});}}'
        )
    ...
    def init_3d_widgets(self):
        ...
        self.item_dict['3d']['pred_human_line_list'] = [[] for _ in range(self.max_num_humans)]
        ...
        for human_idx in range(self.max_num_humans):
            for _ in range(len(self.meta_info['skeleton'])):
                ...
                pred_human_line_item = gl.GLLinePlotItem(
                    pos=np.zeros((2, 3)),
                    color=[c / 255 for c in self.human_colors[human_idx]] + [1.0],
                    width=3,
                    antialias=True,
                )
                self.item_dict['3d']['pred_human_line_list'][human_idx].append(pred_human_line_item)
        ...
        if 'camera' in self.data3d_group:
            ...
            self.item_dict['3d']['camera_axes_list'] = [[] for _ in range(self.max_num_cameras)]
    ...
    def openfile(self):
        filepath_list, filters = QFileDialog.getOpenFileNames(
            self, 'Choose File', '', '3D Data(*.zip *.npz);;Videos (*.mp4 *.avi *.mov)'
        )
        ...
        if len(filepath_list) > 0:
            ...
            if 'zip' in filters.lower():
                assert len(filepath_list) == 1, 'Open just one zip file'
                self.data3d_group = self.read_zarr(filepath_list[0])
                if 'gt3d' in self.data3d_group:
                    self.zarr_len = len(self.data3d_group['gt3d'])
                elif 'pred3d' in self.data3d_group:
                    self.zarr_len = len(self.data3d_group['pred3d'])
                else:
                    assert 0, 'Zarr file does not contain 3d data!'
    ...
    def read_zarr(self, path):
        """ """
        zarr_data = zarr.open_consolidated(path, mode='r')

        # Read data from disk to memory at once, for simplicity
        gt3d = zarr_data['gt3d'][:]  # [F, N, J, 3]
        camera = zarr_data['camera'][:]  # [F, C, J, 3]
        pred3d = zarr_data['pred3d'][:]  # [F, N_max] ragged array of shape [J*3]
        pred3d_filled = np.zeros_like(gt3d)  # [F, N, J, 3]
        for frame_idx in range(gt3d.shape[0]):
            for human_idx in range(gt3d.shape[1]):
                if len(pred3d[frame_idx, human_idx]) == 0:
                    continue
                pred3d_filled[frame_idx, human_idx] = pred3d[frame_idx, human_idx].reshape((-1, 3))

        if 'map' in zarr_data.attrs:
            map_config = get_map_config(zarr_data.attrs['map'])
            map_center = np.array(map_config['map_center'], copy=True)
        else:
            map_center = np.zeros((3,), dtype=np.float32)

        data_dict = {
            'gt3d': gt3d,
            'pred3d': pred3d_filled,
            'camera': camera,
    ...
    def read_numpy(self, path):
        ...
        if 'gt3d' in data_dict:
            assert data_dict['gt3d'].ndim == 4

        if 'pred3d' in data_dict:
            assert data_dict['pred3d'].ndim == 4

        if 'camera' in data_dict:
            assert data_dict['camera'].ndim == 4
    ...
    def update_3d(self):
        ...
        # plot gt 3d & pred 3d human
        gt3d_arr_plot = self.data3d_group['gt3d'][self.current_frame].copy()  # [N, j, 3]
        pred3d_arr_plot = self.data3d_group['pred3d'][self.current_frame].copy()  # [N, j, 3]

        for human_idx in range(self.gt_num_humans):
            for pair, gt_line in zip(
                self.meta_info['skeleton'],
                self.item_dict['3d']['gt_human_line_list'][human_idx],
            ):
                u, v = pair
        ...
        for human_idx in range(self.pred_num_humans):
            if len(pred3d_arr_plot[human_idx]) == 0:
                # unreconstructed person
                # to support zarr data format
                pred3d_this_person = np.zeros(gt3d_arr_plot.shape[1:], dtype=np.float32)
            else:
            ...
            for pair, pred_line in zip(
                self.meta_info['skeleton'],
                self.item_dict['3d']['pred_human_line_list'][human_idx],
            ):
                u, v = pair
                s = pred3d_this_person[u]
                e = pred3d_this_person[v]
                # some joints may not be reconstructed
                if np.any(s) or np.any(e):
                    if not np.any(s):
                        s = e + np.array((1, 1, 1))  # plot e as a samll point
                    if not np.any(e):
                        e = s + np.array((1, 1, 1))
        ...
        if 'camera' in self.data3d_group:
            ...
            for corners_world, camera_axes in zip(
                ...
                corners_world_plot = corners_world.copy()  # [5, 3]
    ...
    def _init_single_plot(self, plot_idx):
        ...
        # curve
        for pen, symbol, name, data in zip(
            self.pens, self.symbols, self.person_names, self.statistics_data[f'p{plot_idx}']
        ):
    ...
    def _update_single_plot(self, plot_idx):
        ...
        for idx, name in enumerate(self.person_names):
            self.item_dict['plot'][f'p{plot_idx}_hLine_' + name].setPos(
                data[idx, self.current_frame]
            )
    ...
    def hide_curve(self, args_list):
        ...
        curve_item = self.item_dict['plot'].get(
            f'p{plot_idx}_curve_' + self.person_names[human_idx]
        )
